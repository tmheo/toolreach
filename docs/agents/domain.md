...
- **`CONTEXT.md`** at the repo root - the domain glossary and ubiquitous language.
...
If this repo ever splits into genuinely separate domains, switch to a multi-context layout: add `CONTEXT-MAP.md` at the root pointing at one `CONTEXT.md` per context, keep system-wide decisions in `docs/adr/`, and put context-scoped decisions in `src/<context>/docs/adr/`.
